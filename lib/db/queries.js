...
export const getLoggedInUser = `
  SELECT id, user_id AS userId, role, username, email 
  FROM users 
  WHERE user_id = ?;
`;
...
export const getEnrolledCourse = `
  SELECT 
    ...
    e.payment_date AS enrollment_date,
...
// Get course with Stripe information
export const getCourseWithStripe = `
  SELECT 
    c.*,
    u.username as instructor_name
  FROM courses c
...
// Create enrollment with Stripe payment intent
export const createEnrollmentWithPayment = `
...
export const updateCourseStripeIds = `
  UPDATE courses 
  SET stripe_product_id = ?, stripe_price_id = ?
  WHERE id = ? AND instructor_id = ?;
`;

// Update course with new Stripe price ID (when price changes)
export const updateCourseStripePrice = `
  UPDATE courses 
  SET stripe_price_id = ?
  WHERE id = ? AND instructor_id = ?;
`;
...
export const updateEnrollmentStatusAfterPayment = `
  UPDATE enrollments 
...
export const getEnrollmentByPaymentIntent = `
...
// Create enrollment without payment intent (for checkout)
export const createPendingEnrollment = `
  INSERT INTO enrollments (user_id, course_id, price, status, enrolled_at) 
  VALUES (?, ?, ?, 'PAYMENT_PENDING', NOW());
`;

// Find pending enrollment by user and course
export const findPendingEnrollmentByUserAndCourse = `
  SELECT * FROM enrollments 
  WHERE user_id = ? AND course_id = ? AND status = 'PAYMENT_PENDING' 
  ORDER BY enrolled_at DESC 
  LIMIT 1;
`;

// Activate enrollment after successful payment
export const activateEnrollmentWithPayment = `
  UPDATE enrollments 
  SET status = 'ACTIVE', 
      stripe_payment_intent_id = ?, 
      payment_date = NOW() 
...
export const markEnrollmentAsFailed = `
  UPDATE enrollments 
  SET status = 'FAILED' 
  WHERE user_id = ? AND course_id = ? AND status = 'PAYMENT_PENDING';
`;

// Get enrollment by ID for verification
