...
export const getAgeVerifiedUser = `
  SELECT id, user_id, role, username, email, oneid_verified, oneid_verification_date 
  FROM users 
  WHERE user_id = ?;`;
...
// ! ONEID VERIFICATION QUERIES

export const getUserWithVerification = `
  SELECT id, user_id, role, username, email, oneid_verified, oneid_verification_date, oneid_verification_data
  FROM users 
  WHERE user_id = ?;
`;
...
export const getVerifiedUsers = `
  ...
  FROM users 
  WHERE oneid_verified = TRUE;
`;

export const getUnverifiedUsers = `
  SELECT COUNT(*) as unverified_count 
  FROM users 
  WHERE oneid_verified = FALSE OR oneid_verified IS NULL;
`;
...
SELECT user_id, username, email, role, entry_date, oneid_verified, oneid_verification_date
     FROM users WHERE id = ?;`;
